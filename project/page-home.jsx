    ...
    <div className="page-enter">
      ...
      <section>
        <div className="container">
          ...
          <div className="grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--gap-grid)' }}>
            {BLOG_POSTS.map((p, i) => (
              <article key={p.id} className="card card-hover" style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ aspectRatio: '4/3', borderRadius: 8, background: ['var(--paper-2)', 'var(--ink)', 'var(--emerald)'][i], marginBottom: 24, position: 'relative', overflow: 'hidden', border: '1px solid var(--rule)' }}>
                  <span className="mono" style={{ position: 'absolute', top: 16, left: 16, fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: i === 0 ? 'var(--ink)' : 'var(--paper)' }}>{p.topic}</span>
                  <div className="serif" style={{ position: 'absolute', bottom: -10, right: 16, fontSize: 110, fontWeight: 400, color: i === 0 ? 'rgba(14,26,23,0.08)' : 'rgba(244,239,227,0.15)', lineHeight: 1, fontStyle: 'italic' }}>0{i+1}</div>
                </div>
                <div className="mono muted mb-3" style={{ fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase' }}>{p.date} · {p.readTime}</div>
                <h3 className="serif" style={{ fontSize: 22, fontWeight: 500, lineHeight: 1.25, marginBottom: 16 }}>{p.title}</h3>
                <a href="#" style={{ marginTop: 'auto', display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 500, color: 'var(--emerald)' }}>Read article <Icon name="arrow-up-right" size={14}/></a>
              </article>
            ))}
          </div>
        </div>
